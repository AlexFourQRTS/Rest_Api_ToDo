    ...
    return games.map(game => {
      ...
      for (const ext of ['.png', '.jpg', '.jpeg']) {
        if (fileSet.has(baseName + ext)) {
          game.hasImage = true;
          game.imagePath = `/api/v1/consoles/${game.console}/images/${encodeURIComponent(baseName + ext)}`;
          break;
        }
      }
      ...
      if (config) {
        for (const ext of config.saveExtensions) {
          if (fileSet.has(baseName + ext)) {
            game.hasSave = true;
            game.savePath = `/api/v1/consoles/${game.console}/saves/${encodeURIComponent(baseName + ext)}`;
            break;
          }
        }
      }
